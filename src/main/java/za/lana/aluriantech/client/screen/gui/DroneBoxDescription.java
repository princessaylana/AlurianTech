...
public class DroneBoxDescription extends SyncedGuiDescription {
	...
	private static final Identifier BUTTON_SEND_DATA = new Identifier(AlurianTech.MOD_ID, "dronebox_button_click");
    ...
    public DroneBoxDescription(ScreenHandlerType<DroneBoxDescription> droneboxDescription, int syncId, PlayerInventory playerInventory, ScreenHandlerContext context) {
		super(AlurianTechScreens.DRONEBOX_GUI, syncId, playerInventory, getBlockInventory(context, INVENTORY_SIZE), null);
		assert player != null;
		...
		root.add(slot, 0, 1);
		// BUTTON
		WButton button = new WButton(Text.translatable("aluriantech_gui.dronebox.sendbutton"));
		button.setOnClick(() -> {
			// SPAWN ENTITY
			ClientPlayNetworking.send(AlurianTechPackets.SPAWN_CARGODRONE_CS_SYNCPACKET, PacketByteBufs.create());
